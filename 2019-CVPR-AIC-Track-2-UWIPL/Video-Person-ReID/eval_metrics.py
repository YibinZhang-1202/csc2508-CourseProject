...
def evaluate_tracklets(distmat, q_pids, g_pids, q_camids, g_camids, q_imgids, g_imgids, max_rank=50, top_N=0):
    '''
    mAP and cmc in per-image basis
    g_imgids, g_imgids: list of list of imgid
    return all_cmc, mAP, and matches_imgids (map from q_imgids to g_imgids)
    ...
    for q_id in range(num_q):
        ...
        for g_id in range(num_g):
            if distmat[q_id][indices[q_id][g_id]] < 0.5:
                ...
                break
        tracklet_ids.append(the_traklet_ids)

    # print('\n', distmat[7][indices[7][0:10]], '\n')
    # print('\n', indices[7][0:10], '\n')
    # print('\n', tracklet_ids, '\n')
